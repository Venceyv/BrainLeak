...
export const PostAbstract: FC<PostAbstractProp> = ({user, post}) => {
  return (
    <div className="flex justify-center gap-3 py-4">
      <div className="flex flex-col items-center justify-center">
        <img
          ...
          alt="user"
        />
        <p className="text-sm cursor-pointer overflow-ellipsis">{user.userName}</p>
      </div>

      <div className="flex flex-col bg-secondary-black w-[300px] rounded-2xl p-3 pb-[1px] border-[1px] border-zinc-400 cursor-pointer">
        <h1 className="text-[14px] font-bold w-full overflow-ellipsis">{post.title}</h1>
        <p className="text-[12px] overflow-ellipsis w-full text-zinc-400">{post.description}</p>
        <span className="flex flex-row gap-2 text-[11px] pt-3">
            <p>Like: {post.likeCount}</p>
            <p>Dislike: {post.dislikeCount}</p>
