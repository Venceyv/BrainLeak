...
interface SortByMenuProp {
  sortBy: SortByType;
  setSortBy: React.Dispatch<React.SetStateAction<SortByType>>;
}

export const SortByMenu: FC<SortByMenuProp> = ({
  sortBy,
  setSortBy,
}): JSX.Element => {
  const buttonRef = useRef<HTMLDivElement>(null);

  const onSetSortBy = (ev: React.ChangeEvent<HTMLSelectElement>) => {
    setSortBy(ev.target.value as SortByType);
  };

  return (
    <div className="flex flex-row justify-end gap-2 pl-4">
      <img
