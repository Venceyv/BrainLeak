    ...
    try {
        ...
        if (dbBack.length != 0) {
            ...
            switch (order) {
                ...
                case 'best':
                    dbBack.sort((a, b) => {
                        if ((b.statistics.likes - b.statistics.dislikes) >
                            (a.statistics.likes - a.statistics.dislikes)) {
                            return 1;
                        }
                        return -1;
                    })
                ...
                case 'hot':
                    dbBack.sort((a, b) => {
                        ...
                        const aCreateTime = (Date.now() 
                        - new Date(a.publishDate))/1000/60;
                        //pubulished til now --minutes
                        const bCreateTime = (Date.now()
                         - new Date(b.publishDate))/1000/60;
...
export {
    createPost, findOne, findByTags, findAll,
    findBySearch, updatePost, deletePost, likePost,
    dislikePost, savePost, postTrending
};