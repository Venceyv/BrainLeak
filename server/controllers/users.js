    ...
    if (!dbBack) {
      let [userInfo, commentList] = await Promise.all([
        ...
        Comment.find({ author: req.params.userId }, { edited: 0, likes: 0 })
          ...
          .populate(
            "author",
            { avatar: 1, username: 1, introduction: 1 },
            { lean: true }
          )
    ...
    if (req.user) {
      ...
      if (req.user._id != req.params.userId) {
        ...
        dbBack.commentList.forEach((comment, index) => {
          dbBack.commentList[index] = addCommentUserInfo(
            comment,
            follwingList,
            commentLikeList
          );
...
async function getLikePosts(req, res) {
  try {
    ...
    if (!dbBack) {
      dbBack = await PostLike.find(
        { user: req.user._id, like: true },
        { _id: 0, post: 1 }
      )
        .lean()
        .populate({
